...
export default function ImageToWord() {
  ...
  const handleUpload = async () => {
    ...
    try {
      const res = await axios.post('http://localhost:5000/api/image-to-word', fd, {
        responseType: 'blob',
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{
          ...
          <h1 style={{ margin: '0 0 10px 0', fontSize: '2.5rem', fontWeight: 'bold' }}>📄 Image → Word</h1>
          <p style={{ margin: '0', fontSize: '1.1rem', opacity: 0.9 }}>Convert images to editable Word documents using OCR</p>
        </div>
        ...
        <div style={{ padding: '40px 30px' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '30px' }}>
            <button
              ...
              style={{
                background: loading || !image ? '#ccc' : 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)',
                color: 'white',
                border: 'none',
                padding: '15px 30px',
                borderRadius: '50px',
                ...
                transition: 'all 0.3s ease',
                boxShadow: loading || !image ? 'none' : '0 4px 15px rgba(102, 126, 234, 0.4)'
              }}
            >
